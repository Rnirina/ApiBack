...
import mg.itu.tpt.services.ServiceEtat;
...
public class Etat extends HttpServlet {
    private ServiceEtat services;
    public void init(ServletConfig config) throws ServletException  {
        super.init(config);
        try {
            PoolConnection pool = new PoolConnection();
            services = new ServiceEtat(pool);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Etat</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Etat at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
  ...
  String json = "";
        try {
            if (request.getParameter("idEtat") != null) {
                json = new Gson().toJson(this.services.getUn(Integer.parseInt(request.getParameter("idEtat"))));
            } else {
